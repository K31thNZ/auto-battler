...
// ─── Heal creatures ───────────────────────────────────────────────────────────
// Heal all player creatures (free, with cooldown) or pay Shards for instant heal
router.post('/heal', authMiddleware, (req, res) => {
  const { playerCreatureIds, useShards } = req.body;

  const player = db.prepare('SELECT * FROM players WHERE id = ?').get(req.user.playerId);

  // Fetch the creatures to heal (if ids provided, heal those; else heal all)
  let query = `
    SELECT pc.*, c.base_hp
    FROM player_creatures pc JOIN creatures c ON pc.creature_id = c.id
    WHERE pc.player_id = ?
  `;
  const params = [req.user.playerId];
  if (playerCreatureIds && playerCreatureIds.length > 0) {
    query += ` AND pc.id IN (${playerCreatureIds.map(() => '?').join(',')})`;
    params.push(...playerCreatureIds);
  }
  const creatures = db.prepare(query).all(...params);

  const woundedCreatures = creatures.filter(pc => pc.current_hp < pc.max_hp);
  if (woundedCreatures.length === 0) {
    return res.json({ success: true, message: 'All creatures are already at full health!', healed: [] });
  }

  // Calculate shard cost: 5 shards per creature that needs healing
  const shardCost = useShards ? woundedCreatures.length * 5 : 0;

  if (useShards && player.shards < shardCost) {
    return res.status(400).json({ error: `Not enough Shards. Need ${shardCost} 💎 to heal ${woundedCreatures.length} creatures.` });
  }

  // Perform heal
  const healCreature = db.prepare('UPDATE player_creatures SET current_hp = max_hp WHERE id = ? AND player_id = ?');
  const healAll = db.transaction(() => {
    for (const pc of woundedCreatures) {
      healCreature.run(pc.id, req.user.playerId);
    }
    if (useShards && shardCost > 0) {
      db.prepare('UPDATE players SET shards = shards - ? WHERE id = ?').run(shardCost, req.user.playerId);
    }
  });
  healAll();
  ...
  res.json({
    success: true,
    healed: woundedCreatures.map(pc => pc.id),
    count: woundedCreatures.length,
    shardCost,
    newShards: updatedPlayer.shards,
    message: useShards
      ? `Healed ${woundedCreatures.length} creatures for ${shardCost} 💎`
      : `Healed ${woundedCreatures.length} creatures`,
  });
});
